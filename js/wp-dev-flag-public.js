(function ($) {
	...
	$(document).ready(
		function () {
			...
			var $html = '<div class="' + ((wp_dev_flag_options.custom_class) ? wp_dev_flag_options.custom_class + ' ' : '') + 'wp-dev-flag ' + wp_dev_flag_options.horizontal + ' ' + wp_dev_flag_options.vertical + '">';
			if ( wp_dev_flag_options.url ) {
				$html += '<a href="' + wp_dev_flag_options.url + '" ' + ((wp_dev_flag_options.open_in_new_tab) ? 'target="_blank"' : '') + '>';
			}
			$html += wp_dev_flag_options.message
			if ( wp_dev_flag_options.url ) {
				$html += '</a>';
			}
			$html += '</div>';
