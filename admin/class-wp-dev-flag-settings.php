...
class Wp_Dev_Flag_Settings {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->stored_environment = ( get_option( 'wp_dev_flag_trigger_options' ) ) ? unserialize( get_option( 'wp_dev_flag_trigger_options' )['dev_environment'] ) : [];
	...
	public function render_settings_page_content( $active_tab = '' ) {
		...
		} elseif ( 'link_options' === $active_tab ) {
		...
		?>
			...
			<h2 class="nav-tab-wrapper">
				...
				<a href="?page=wp_dev_flag_options&tab=link_options" class="nav-tab <?php echo 'link_options' === $active_tab ? 'nav-tab-active' : ''; ?>"><?php _e( 'Link Options', 'wp-dev-flag-plugin' ); ?></a>
			</h2>
		...
		if ( 'trigger_options' === $active_tab ) {
			...
			settings_fields( 'wp_dev_flag_link_options' );
			do_settings_sections( 'wp_dev_flag_link_options' );
	...
	/**
	 * This function provides a simple description for the Link Options page.
	 *
	 * It's called from the 'initialize_display_options' function by being passed as a parameter
	 * in the add_settings_section function.
	 */
	public function link_options_callback() {
		echo '<p>' . __( 'Set the URL & link behaviour of your WP Dev Flag badge.', 'wp-dev-flag-plugin' ) . '</p>';
	}
	...
	public function initialize_display_options() {
		...
		// The checkbox to determine if you want to show the badge in the dashboard.
		add_settings_field(
			'show_in_admin',                                        // ID used to identify the field throughout the theme
			__( 'Show badge in admin?', 'wp-dev-flag-plugin' ), // The label to the left of the option interface element
			array( $this, 'show_in_admin_callback' ),               // The name of the function responsible for rendering the option interface
			'wp_dev_flag_display_options',                          // The page on which this option will be displayed
			'display_settings_section',                             // The name of the section to which this field belongs
			array(                                                  // The array of arguments to pass to the callback. In this case, just a description.
				__( 'This will add a badge to the admin bar.', 'wp-dev-flag-plugin' ),
			)
		);

		// The checkbox to determine if you want to show the badge on the front end.
		add_settings_field(
			'show_on_frontend',                                        // ID used to identify the field throughout the theme
			__( 'Show badge on front end?', 'wp-dev-flag-plugin' ), // The label to the left of the option interface element
			array( $this, 'show_on_frontend_callback' ),               // The name of the function responsible for rendering the option interface
			'wp_dev_flag_display_options',                          // The page on which this option will be displayed
			'display_settings_section',                             // The name of the section to which this field belongs
			array(                                                  // The array of arguments to pass to the callback. In this case, just a description.
				__( 'This will add the badge to the front end of your site.', 'wp-dev-flag-plugin' ),
			)
		);
	...
	/**
	 * Initializes the theme's link options page by registering the Sections,
	 * Fields, and Settings.
	 *
	 * This function is registered with the 'admin_init' hook.
	 */
	public function initialize_link_options() {

		// First, we add a settings section to contain all the settings fields.
		add_settings_section(
			'link_settings_section',                    // ID used to identify this section and with which to register options
			__( 'Link Options', 'wp-dev-flag-plugin' ), // Title to be displayed on the administration page
			array( $this, 'link_options_callback' ),    // Callback used to render the description of the section
			'wp_dev_flag_link_options'                  // Page on which to add this section of options
		);

        // The link URL
        add_settings_field(
            'url',
            __( 'Link URL', 'wp-dev-flag-plugin' ),
            array( $this, 'url_callback' ),
            'wp_dev_flag_link_options',
            'link_settings_section',
            array(                                                  // The array of arguments to pass to the callback. In this case, just a description.
                __( 'Enter the full URL including <strong>https://</strong> eg. <strong>https://chrisjallen.com</strong>.', 'wp-dev-flag-plugin' ),
            )
        );

		// The checkbox to determine if you want to open the link in a new tab.
		add_settings_field(
			'open_in_new_tab',                                   // ID used to identify the field throughout the theme
			__( 'Open link in new tab?', 'wp-dev-flag-plugin' ), // The label to the left of the option interface element
			array( $this, 'open_in_new_tab_callback' ),          // The name of the function responsible for rendering the option interface
			'wp_dev_flag_link_options',                          // The page on which this option will be displayed
			'link_settings_section',                             // The name of the section to which this field belongs
			array(                                                  // The array of arguments to pass to the callback. In this case, just a description.
				__( 'This will set the link to open in a new tab when clicked.', 'wp-dev-flag-plugin' ),
			)
		);
		...
		register_setting(
			'wp_dev_flag_link_options',
			'wp_dev_flag_link_options',
            array( $this, 'sanitize_link_options' )
        );

	} // end initialize_link_options
	...
	public function update_environment_callback( $args ) {
		...
		$html = '<input type="checkbox" id="update_environment" name="wp_dev_flag_trigger_options[update_environment]" value="1" ' . ( ( isset( $options['update_environment'] ) ) ? 'checked="checked"' : '' ) . ' />';
	...
	/**
	 * This function renders the checkbox field to indicate whether you want to show the badge in admin or not.
	 *
	 */
	public function show_in_admin_callback( $args ) {

		// First, we read the options collection
		$options = get_option( 'wp_dev_flag_display_options', [] );

		// Generate a checkbox and set its default checked/unchecked state.
		$html = '<input type="checkbox" id="show_in_admin" name="wp_dev_flag_display_options[show_in_admin]" value="1" ' . ( ( isset( $options['show_in_admin'] ) ) ? 'checked="checked"' : '' ) . ' />';

		// Here, we'll take the first argument of the array and add it to a label next to the checkbox
		$html .= '<label for="show_in_admin">&nbsp;' . $args[0] . '</label>';
	 ...
	 *
	 */
	public function show_on_frontend_callback( $args ) {

		// First, we read the options collection
		$options = get_option( 'wp_dev_flag_display_options', [] );

		// Generate a checkbox and set its default checked/unchecked state.
		$html = '<input type="checkbox" id="show_on_frontend" name="wp_dev_flag_display_options[show_on_frontend]" value="1" ' . ( ( isset( $options['show_on_frontend'] ) ) ? 'checked="checked"' : '' ) . ' />';

		// Here, we'll take the first argument of the array and add it to a label next to the checkbox
		$html .= '<label for="show_on_frontend">&nbsp;' . $args[0] . '</label>';
    ...
    public function url_callback( $args ) {
        ...
        $options = get_option( 'wp_dev_flag_link_options', [] );

        $html = '<input name="wp_dev_flag_link_options[url]" type="text" value="' . ( ( isset( $options['url'] ) ) ? $options['url'] : '' ) . '" class="url" />';

        $html .= '<label for="url">&nbsp;' . $args[0] . '</label>';
    ...
    /**
     * This function renders the checkbox field to indicate whether you want the link to open in a new tab
     *
     */
    public function open_in_new_tab_callback( $args ) {

        // First, we read the options collection
        $options = get_option( 'wp_dev_flag_link_options', [] );

        // Generate a checkbox and set its default checked/unchecked state.
        $html = '<input type="checkbox" id="open_in_new_tab" name="wp_dev_flag_link_options[open_in_new_tab]" value="1" ' . ( ( isset( $options['open_in_new_tab'] ) ) ? 'checked="checked"' : '' ) . ' />';

        // Here, we'll take the first argument of the array and add it to a label next to the checkbox
        $html .= '<label for="open_in_new_tab">&nbsp;' . $args[0] . '</label>';

        echo $html;

    } // open_in_new_tab_callback
    ...
    /**
     * Sanitization callback for the link options. Since some of the link options are text inputs,
     * this function loops through the incoming option and strips all tags and slashes from the values
     * before serializing it.
     *
     * @params $input  The unsanitized collection of options.
     ...
     * @returns The collection of sanitized values.
     */
    public function sanitize_link_options( $input ) {
        ...
        // Return the new collection
        return apply_filters( 'sanitize_link_options', $output, $input );

    } // end sanitize_link_options
