...
class Wp_Dev_Flag_Public {
	...
	public function enqueue_scripts() {
		...
		if ( get_option( 'wp_dev_flag_show_flag' ) && isset( $display_options['show_on_frontend'] ) ) {
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/wp-dev-flag-public.js', array( 'jquery' ), $this->version, false );
			wp_localize_script( $this->plugin_name, 'wp_dev_flag_options', array_merge( get_option( 'wp_dev_flag_trigger_options', [] ), get_option( 'wp_dev_flag_display_options', [] ), get_option( 'wp_dev_flag_link_options', [] ) ) );
		}
